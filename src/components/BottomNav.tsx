...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-50 bg-slate-900 border-t border-slate-800 safe-bottom">
        ...
        {tabs.map(({ to, label, icon: Icon, center }) =>
          center ? (
            <button
              key={to}
              onClick={() => navigate(to)}
              className="flex flex-col items-center justify-center -mt-5 w-14 h-14 rounded-full bg-blue-600 shadow-lg shadow-blue-900/50 active:scale-95 transition-transform"
              aria-label={label}
            >
